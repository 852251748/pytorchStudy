...
class GenData:
    def __init__(self, root, imgsize):
        self.imgpath = r"E:\BaiduNetdiskDownload\深度学习\celebA\CelebA\Img\img_celeba.7z\img_celeba"
        self.lableBoxPath = r"E:\BaiduNetdiskDownload\深度学习\celebA\CelebA\Anno\list_bbox_celeba.txt"
        self.lableLandMaPath = r"E:\BaiduNetdiskDownload\深度学习\celebA\CelebA\Anno\list_landmarks_celeba.txt"
        ...
        for i, lable in enumerate(open(self.lableBoxPath)):
            if i > 1:
                ...
                for z in range(epoch):
                    # 生成正样本点
                    sampCx, sampCy = cx + np.random.randint(int(-w * 0.2), int(w * 0.2)), \
                                     cy + np.random.randint(int(-h * 0.2), int(h * 0.2))
                ...
                for k in range(len(poList)):
                    ...
                    # 计算样本点IOU
                    iou = IouTorch(torch.tensor([x1, y1, x2, y2]).float(), torch.tensor([poList[k][:4]]).float())
...
if __name__ == '__main__':
    gendata = GenData("D:/pycharm_workspace/mtcnn_data", 48)
    gendata(10)
