...
class Detector:
    ...
    def __init__(self, pnet_param="./pnet.pt", rnet_param="./rnet.pt", onet_param="./onet.pt",
                 isCuda=True):

        self.isCuda = isCuda

        self.pnet = PNet()  # 创建实例变量，实例化P网络
        self.rnet = RNet()
        self.onet = ONet()
    ...
    def __pnet_detect(self, image):  # ★p网络全部是卷积，与输入图片大小无关，可输出任意形状图片
        ...
        while min_side_len > 12:  # 直到缩放到小于等于12时停止
            ...
            # print("_cls", _cls.shape, _cls)  # [1, 1, 203, 245]:NCWH：分组卷积的特征图的通道和尺寸★
        ...
        return nms(np.array(boxes), p_nms)  # 返回框框，原阈值给p_nms=0.5（iou为0.5），尽可能保留IOU小于0.5的一些框下来，若网络训练的好，值可以给低些
    ...
    def __rnet_detect(self, image, pnet_boxes):
        ...
        _pnet_boxes = convert_to_square(pnet_boxes)  # ★给p网络输出的框，找出中心点，沿着最大边长的两边扩充成“正方形”，再抠图
        ...
        return nms(np.array(boxes), r_nms)  # 原r_nms为0.5（0.5要往小调），上面的0.6要往大调;小于0.5的框被保留下来

    # 创建O网络检测函数
    def __onet_detect(self, image, rnet_boxes):

        _img_dataset = []  # 创建列表，存放抠图r
        _rnet_boxes = convert_to_square(rnet_boxes)  # 给r网络输出的框，找出中心点，沿着最大边长的两边扩充成“正方形”
        ...
        return nms(np.array(boxes), o_nms, isMin=True)  # 用最小面积的IOU；原o_nms(IOU)为小于0.7的框被保留下来


if __name__ == '__main__':
    # 多张图片检测
    image_path = r"newimage"
    newimage_path = r"newimage"
    for i in os.listdir(newimage_path):
        detector = Detector()
        with Image.open(os.path.join(newimage_path, i)) as im:  # 打开图片
